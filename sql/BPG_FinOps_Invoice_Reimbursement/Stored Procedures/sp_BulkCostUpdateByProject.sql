...
-- =============================================
CREATE PROCEDURE [BPG_FinOps_Invoice_Reimbursement].[sp_BulkCostUpdateByProject]
(
...
BEGIN
	...
	INSERT INTO [BPG_FinOps_Invoice_Reimbursement].[Log]
           ...
           ,[Description])
     VALUES
           ('[BPG_FinOps_Invoice_Reimbursement].[sp_BulkCostUpdateByProject]'
           ,'Begin args: @ProjectIds = ''' 
		    + CONVERT(NVARCHAR(MAX), @ProjectIds) + ''' @QuarterId = '''
	...
	begin
		...
		where trans.QuarterId = @QuarterId and trans.ProjectId in (
				select Id from OPENJSON(@ProjectIds)
				WITH (
