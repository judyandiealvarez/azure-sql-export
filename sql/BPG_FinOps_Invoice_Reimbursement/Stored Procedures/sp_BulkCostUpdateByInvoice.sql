...
-- =============================================
CREATE PROCEDURE [BPG_FinOps_Invoice_Reimbursement].[sp_BulkCostUpdateByInvoice]
(
    @ProposalIds nvarchar(MAX),
	@QuarterId nvarchar(200),
	...
	@CorrectReason nvarchar(50) = NULL,
...
BEGIN
	...
	INSERT INTO [BPG_FinOps_Invoice_Reimbursement].[Log]
           ...
           ,[Description])
     VALUES
           ('[BPG_FinOps_Invoice_Reimbursement].[sp_BulkCostUpdateByInvoice]'
           ,'Begin args: @ProposalIds = ''' 
		    + CONVERT(NVARCHAR(MAX), @ProposalIds) +''' @QuarterId = '''
		    + isnull(CONVERT(NVARCHAR(150), @QuarterId), 'NULL')+ ''' @IsSubmitted = '''
	...
	begin
		...
		set 
			...
			SentForApprovalDate = case when @SendDate IS NULL then GETUTCDATE() else @SendDate end
		from [BPG_FinOps_Invoice_Reimbursement].[TransactionCost] trans
		where trans.QuarterId = @QuarterId and trans.ProposalId in (
				select Id from OPENJSON(@ProposalIds)
				WITH (
					Id NVARCHAR(50) 'strict $.Id'
				)
			)
			and SentForApproval = 1
