...
-- =============================================
CREATE PROCEDURE [BPG_FinOps_Invoice_Reimbursement].[sp_BulkCostUpdateByInvoiceJSON]
(
    @json nvarchar(MAX)
)
AS
...
--}
	...
	INSERT INTO [BPG_FinOps_Invoice_Reimbursement].[Log]
           ...
           ,[Description])
     VALUES
           ('[BPG_FinOps_Invoice_Reimbursement].[sp_BulkCostUpdateByInvoiceJSON]'
           ,@json)
	...
	begin
		...
		where trans.QuarterId = @QuarterId and trans.ProposalId in (
				select Id from OPENJSON(JSON_QUERY(@json, '$.ProposalIds'))
				WITH (
